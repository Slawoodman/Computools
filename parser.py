...
parse_type = int(input('Your choice: 1 - parse all chat, 2 - parse one user: '))
if parse_type == 1:
		...
		try:
			...
			if r.status_code == 200 and len(loads(r.text)) > 1:
				for usermessage in loads(r.text):
					...
					if len(current_message) > 0 and current_message != ' ' and current_message != ' ' and current_message.count(' ') >= min_words-1:
						check_re = search(r'<.*?>', current_message)
						if check_re == None:
							if match('^[a-zA-Zа-яА-ЯёЁ 0-9 ?.-_!()]+$', current_message) is not None:
